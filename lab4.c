...
int main(void) {
	...
	//led_init();
	P1DIR |= BIT0 | BIT4; // настройка светодиодов на выход
    P8DIR |= BIT1 | BIT2;
    ...
    ADC12MCTL0 |= ADC12INCH_10;	// temper
    // channel 1 config (select temp as source)
    //set channel 1 as last for adc
    ADC12MCTL1 |= ADC12EOS;
    ADC12MCTL1 |= ADC12INCH_9;	//Vref

	// Comp init

    CBCTL0 |= CBIMEN + CBIMSEL_0;	// разрешение входного канала V- и выбор входного канала pad4
    CBCTL0 |= CBIPEN + CBIPSEL_5;	// разрешение входного канала V+ и выбор входного канала потенциометра
    ...
    P6DIR |= BIT4 | BIT1 | BIT2 | BIT3;
    P6OUT |= BIT4 | BIT1 | BIT2 | BIT3;
...
__interrupt void comparator_interrupt() {
   ...
   if(CBCTL1 & CBOUT) { // если на выходе компаратора 1, то + > -
       P1OUT |= BIT4;	// зажигаем pad4
       P8OUT &= ~BIT2;
   }
   else {
	   P1OUT &= ~BIT4;	// зажигаем led3
	   P8OUT |= BIT2;
   }
...
__interrupt void adc_interrupt() {
    ...
    short val_temp = ADC12MEM0;	// читаем значение с теромодатчика

    if(val_UREF > val_temp) {		// если Vref больше
